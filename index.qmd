...
state_abortion_data <- state_abortion_data %>%
  ...
  mutate(
    ...
    Ratio = ifelse(Ratio %in% c("N/A", 
                                ...
                                "---***", 
                                "---†††"), NA, Ratio),
    `Out-of-State (number and percentage)` = ifelse(`Out-of-State (number and percentage)` %in% c("N/A",
      "—¶¶",
      "—***", 
      "—", 
      "—†††", 
      ...
      "--***", 
      "---***", 
      "---†††"), NA, `Out-of-State (number and percentage)`)
  )  %>%
...
state_economic_data2020 <- state_economic_data2020[-c(1, 10), ]
  plot_usmap(data = state_economic_data2020, values = `Median Household Income`)
...
econ_animate <- ggplot() +
  geom_polygon(data = merged_data, aes(x = long, y = lat, group = group, fill = `Median Household Income`)) +
  scale_fill_gradient(name = "Data", low = "lightblue", high = "darkblue", na.value = "grey", guide = "legend") +
  theme_void() +
  labs(title = "Your Title", caption = "Source: Your Source") +
  transition_time(Year) %>%
  labs(title = "Year: {frame_time}") +  # Add a label showing the current year
...
econ_animate <- ggplot(merged_data, aes(x = long, y = lat, group = group)) +
