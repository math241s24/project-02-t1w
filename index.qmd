...
election_animate <- ggplot(election_merged_data, aes(x = long, y = lat, group = group)) +
  ...
  scale_fill_gradient(name = "Elections", low = "pink", high = "darkred", na.value = "grey", guide = "legend") +
  theme_void() +
  labs(subtitle = "Test", caption = "Source: Test") +
  transition_states(YEAR) +
  labs(title = "Year: {closest_state}") + 
  theme(plot.title = element_text(hjust = 0.5))  
...
```{r}
editing <- all_variables_dataset %>% filter(YEAR == '2008')
