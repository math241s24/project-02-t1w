...
state_abortion_data <- state_abortion_data %>%
  ...
  select(Area, Abortions_per_1000_women, Abortions_per_1000_live_births, `Out_of_state_abortions(number and percentage)`, Year)
```
...
abortion_animate <- ggplot(abortion_merged_data, aes(x = long, y = lat, group = group)) +
  ...
  theme_void() +
  labs(subtitle = "Abortions by State Residents, per 1000 women aged 15-44, over Time", caption = "Source: CDC") +
  transition_states(Year) +
  labs(title = "Year: {closest_state}") + 
  theme(plot.title = element_text(hjust = 0.5))  
